...
RSpec.describe TripService, :vcr do
  it 'posts a trip' do
    ...
    expect(result[:data][:attributes][:resort_id]).to eq(2)
  end
  ...
  it 'returns a trip' do
    ...
    expect(result.first).to be_a(Array)
    expect(result[:data][:attributes][:start_date]).to eq('2021-11-08')
    expect(result[:data][:attributes][:end_date]).to eq('2021-11-25')
  end
  ...
  it 'updates a trip' do
    ...
    expect(result[:data][:attributes]).to have_key :start_date
    expect(result[:data][:attributes]).to have_key :end_date
  end
